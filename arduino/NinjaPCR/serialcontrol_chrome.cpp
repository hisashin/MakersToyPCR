...
SerialControl::SerialControl(Display* pDisplay) :
    ipDisplay(pDisplay), packetState(STATE_START), packetLen(0), packetRealLen(
...
char* SerialControl::AddParam(char* pBuffer, char key, unsigned long val,
    boolean init) {
  if (!init)
    *pBuffer++ = '&';
  *pBuffer++ = key;
...
char* SerialControl::AddParam(char* pBuffer, char key, float val,
...
char* SerialControl::AddParam(char* pBuffer, char key, const char* szVal,
    boolean init) {
  if (!init)
    *pBuffer++ = '&';
  *pBuffer++ = key;
...
char* SerialControl::AddParam_P(char* pBuffer, char key, const char* szVal,
    boolean init) {
  if (!init)
    *pBuffer++ = '&';
  *pBuffer++ = key;
...
const char* SerialControl::GetProgramStateString_P(
...
const char* SerialControl::GetThermalStateString_P(
